  ...
  return (
    <div className="flex min-h-screen" onMouseMove={handleMouseMove}>
      <div className="flex-1 bg-black text-white py-[10vh]">
        <div className="flex flex-col px-36 h-full">
          <motion.div
            ref={ref}
          ...
          <div className="mt-25 h-full border-t border-b border-white">
            {projectsList.map((project, index) => (
              <>
                <div
                  key={index}
                  className="flex group justify-between items-center px-20 h-[305px] border-b border-white cursor-pointer"
                  onMouseEnter={() => handleMouseEnter(index)}
                  onMouseLeave={handleMouseLeave}
                >
                  <div className="text-8xl font-extrabold group-hover:text-gray-400 group-hover:translate-x-[-10px] transition-all duration-150">
                    {project.name}
                  </div>
                </div>
              </>
